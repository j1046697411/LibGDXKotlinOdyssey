...
class LongFastList(capacity: Int = 7) : AbstractMutableFastList<Long>(), LongMutableFastList {
    ...
    override fun unsafeListEditor(): ListEditor<Long> {
        return object : ListEditor<Long> {
            override fun unsafeInsertLast(element: Long) {
                data[size++] = element
            }

            override fun unsafeSet(index: Int, element: Long) {
                data[index] = element
            }
        }
    }

    override fun migrate(index: Int, count: Int, callback: InsertEditor<Long>.() -> Unit) {
