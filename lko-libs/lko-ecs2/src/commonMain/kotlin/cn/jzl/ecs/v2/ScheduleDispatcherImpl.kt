...
class ScheduleDispatcherImpl : ScheduleDispatcher {
    ...
    private val priorityFrameTasks = ObjectFastList<FrameTask>(256)
     ...
     */
    override fun addMainTask(scheduleDescriptor: ScheduleDescriptor, priority: SchedulePriority, task: (Duration) -> Unit) = lock.withLock {
        mainTasks.insertLast(FrameTask(scheduleDescriptor, if (priority == SchedulePriority.Auto) ScheduleTaskPriority.NORMAL else priority, task))
    }
    ...
    override fun addWorkTask(
        ...
        addWorkTask(FrameTask(scheduleDescriptor, if (priority == SchedulePriority.Auto) ScheduleTaskPriority.NORMAL else priority, task))
    }
    ...
    override fun addDelayFrameTask(
        ...
        delayTasks.insertLast(
            DelayFrameTask(
                delay,
                FrameTask(scheduleDescriptor, if (priority == SchedulePriority.Auto) ScheduleTaskPriority.NORMAL else priority, task)
            )
        )
    }
    ...
    private fun executeMainTasks(delta: Duration) {
        ...
        currentFrameTasks.sortBy { it.priority.priority }
        // 执行所有任务
    ...
    private fun executeWorkTasks(delta: Duration) {
        ...
        // 清空工作任务列表，准备接收新任务
        workTasks.clear()
        for (priority in ScheduleTaskPriority.entries) {
            priorityFrameTasks.clear()
            frameTaskExecutor.executePendingTasks(currentFrameTasks.filterTo(priorityFrameTasks) { it.priority == priority }, delta)
        }
        // 执行完成后清空当前帧任务列表
        currentFrameTasks.clear()
        priorityFrameTasks.clear()
