 ...
 */
class EntityService internal constructor(private val world: World) {
    /**
     * 实体存储服务 - 负责实体ID和版本管理
     */
...
suspend inline fun <R> ScheduleScope.batch(
    schedulePriority: SchedulePriority = SchedulePriority.Auto,
    noinline configuration: EntityService.() -> R
): R = suspendScheduleCoroutine(ScheduleScope.DispatcherType.Main) { continuation ->
    continuation.scheduleDispatcher.addMainTask(scheduleDescriptor, schedulePriority) {
        continuation.resumeWith(runCatching { world.entityService.configuration() })
    }
}

suspend inline fun ScheduleScope.create(
    noinline configuration: EntityCreateContext.(Entity) -> Unit
): Entity = batch { create(configuration) }

suspend inline fun ScheduleScope.create(
    entityId: Int,
    noinline configuration: EntityCreateContext.(Entity) -> Unit
): Entity = batch { create(entityId, configuration) }

suspend inline fun <T> ScheduleScope.batchCreation(
    data: Sequence<T>,
    noinline configuration: EntityService.(T) -> Entity
): List<Entity> = batch { data.map { configuration(it) }.toList() }

suspend inline fun ScheduleScope.configure(
    entity: Entity,
    noinline configuration: EntityUpdateContext.(Entity) -> Unit
): Unit = batch { configure(entity, configuration) }

suspend inline fun ScheduleScope.batchConfiguration(
    data: Sequence<Entity>,
    noinline configuration: EntityUpdateContext.(Entity) -> Unit
): Unit = batch { data.forEach { configure(it, configuration) } }

suspend inline fun ScheduleScope.batchConfiguration(
    family: Family,
    noinline configuration: EntityUpdateContext.(Entity) -> Unit
): Unit = batchConfiguration(family.entities, configuration)

suspend fun ScheduleScope.batchRemoval(data: Sequence<Entity>): Unit = batch { data.forEach { remove(it) } }
suspend fun ScheduleScope.remove(entity: Entity): Boolean = batch { remove(entity) }
