...
@PublishedApi
internal class EntityService(override val world: World) : WorldOwner {

    val entityRecords = LongFastList()

    private val destroyTasks = ObjectFastList<() -> Unit>(64)

    private fun addDestroyTask(task: () -> Unit) {
        destroyTasks.add(task)
    }

    fun update() {
        destroyTasks.forEach { it() }
        destroyTasks.clear()
    }
    ...
    inline fun postCreate(entity: Entity, event: Boolean = true, configuration: EntityCreateContext.(Entity) -> Unit): Entity = entity.also {
        ...
        }.onFailure { destroy(entity) }.getOrThrow()
    }
    ...
    inline fun <R> runOn(entity: Entity, block: Archetype.(Int) -> R): R {
        ...
        check(entity in world.entityStore) { "entity id $entity is not valid" }
    ...
    fun destroy(entities: Entities) {
        ...
        entities.forEach { if (isActive(it)) awaitDeletedEntities.add(it) }
        if (awaitDeletedEntities.isNotEmpty()) {
            batchDestroy(awaitDeletedEntities)
        }
    }

    private fun batchDestroy(entities: Entities) = addDestroyTask {
        var index = 0
        // 收集需要删除的实体
